...
export type TransitionFn = () => void;

const defaultLookup: ElementLookup = (id) => {
    if (typeof document === 'undefined') return null;
    return document.getElementById(id);
};
let globalLookup: ElementLookup = defaultLookup;

export const setGlobalLookup = (lookup: ElementLookup) => {
    globalLookup = lookup;
};

export const getGlobalLookup = (): ElementLookup => globalLookup;

/**
 * Queue synchronous transitions to avoid re-entrancy issues; executes in FIFO order.
 * This is opt-in and does not schedule across frames.
 */
export const queueTransitions = (fns: TransitionFn[]) => {
    for (const fn of fns) {
        fn();
    }
};
...
export class StateVar<T> extends baseVar<T> {
    constructor(
        ...
        private readonly lookup: ElementLookup = globalLookup,
    ) {
        super(state_0);
    }
    public override transition(nextState: T) {
        super.transition(nextState);
        const elem = this.lookup(this.id);
        if (elem) {
...
export class StyleVar<T> extends baseVar<T> {
    public stateToStyle: styleFunction<T> = (state: T) => `${state}`;
    constructor(
        state_0: T,
        public readonly id: string,
    ...
    public override transition(nextState: T) {
        ...
        const elem = this.lookup(this.id);
        if (elem) {
            elem.style.setProperty(this.prop, this.stateToStyle(this.current()));
        }
    }
}
...
export class MultiStateVar<T> extends baseVar<T> implements StateBinder<T> {
    ...
    constructor(state_0: T, private readonly lookup: ElementLookup = globalLookup) {
        super(state_0);
    }
    public override transition(nextState: T) {
        super.transition(nextState);
        this.bindings.forEach((funct, id) => {
            const elem = this.lookup(id);
            if (elem) {
    ...
    public removeBinding(id: string) {
...
export class MultiStyleVar<T> extends baseVar<T> implements StyleStateBinder<T> {
    ...
    public override transition(nextState: T) {
        ...
        this.bindings.forEach((prop, id) => {
            const elem = this.lookup(id);
            if (elem) {
                elem.style.setProperty(prop, this.stateToStyle(this.current()));
            }
        });
    }
...
export class StateVarList<T> {
    private ids: string[] = [];
    private states: T[] = [];
    constructor(id_states: { id: string; state: T }[], public readonly funct: HTMLUpdateFunction<T>, private readonly lookup: ElementLookup = globalLookup) {
        id_states.map((id_state) => {
    ...
    public transition(index: number, nextState: T) {
        if (index >= 0 && index < this.ids.length) {
            ...
            const elem = this.lookup(this.ids[index] ?? '');
            if (elem) {
...
export class StyleVarList<T> extends StateVarList<T> {
    public stateToStyle: styleFunction<T>;
    constructor(
        id_states: { id: string; state: T }[],
        prop: string,
        state2style?: styleFunction<T>,
        lookup: ElementLookup = globalLookup,
    ) {
        const resolvedStateToStyle: styleFunction<T> = state2style ?? ((state: T) => `${state}`);
        const funct = (elem: HTMLElement, state: T) => {
            elem.style.setProperty(prop, resolvedStateToStyle(state));
        };
        super(id_states, funct, lookup);
        this.stateToStyle = resolvedStateToStyle;
    }
}
